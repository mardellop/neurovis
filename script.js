/**
 * NeuroVis — Script de Producción FINAL (Corrección de Ruta de Modelos)
 * Este archivo soluciona el error "Failed to fetch" asegurando la carga de modelos.
 */
...
class FaceAnalyzer {
  ...
  async loadModels() {
    ...
    if (window.location.protocol === 'file:') {
      throw new Error("El análisis facial requiere ejecución mediante un servidor (GitHub Pages, Live Server, etc.). No funcionará abriendo el archivo localmente.");
    }

    // Calculamos la ruta absoluta de los modelos basada en la ubicación del index.html
    const MODEL_URL = window.location.origin + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')) + '/models';
    
    console.log('[NeuroVis] Cargando modelos desde:', MODEL_URL);
    ...
    try {
      ...
      console.log('[NeuroVis] Modelos cargados con éxito');
    } catch (e) {
      console.error('[NeuroVis] Fallo al cargar modelos:', e);
      throw new Error(`Error: No se pudieron cargar los modelos de IA de la ruta: ${MODEL_URL}. Asegúrate de que la carpeta 'models' existe y es accesible.`);
    }
  }
  ...
  async analyzeVideo(video, duration, intervalMs = 200, onProgress = null, onFrameResult = null) {
    ...
    for (let t = 0; t <= duration; t += intervalSec) {
      ...
      // Aseguramos que el video esté en el frame correcto
      await new Promise(r => {
        const seeked = () => {
          video.removeEventListener('seeked', seeked);
          // Doble requestAnimationFrame para asegurar que el buffer de video se ha actualizado
...
// ═══════════════ CLASE METRICS ENGINE ═══════════════
class MetricsEngine {
  constructor() { this.reset(); }
  reset() { 
    this.metrics = []; 
    this.averages = { engagement: 0, valence: 0, arousal: 0, dominant: 'neutral' }; 
    this._sums = { e: 0, v: 0, a: 0 }; 
    this._count = 0; 
    this._emotions = {}; 
  }
  ...
  processFrame(frame) {
    ...
    if (frame.faceDetected && frame.expressions) {
      ...
      const pos = (e.happy || 0) + (e.surprised || 0) * 0.5;
      const neg = (e.sad || 0) + (e.angry || 0) + (e.disgusted || 0) + (e.fearful || 0);
      point.valence = Math.max(-1, Math.min(1, pos - neg));
      point.arousal = Math.min(1, (e.happy * 0.7) + (e.surprised) + (e.angry * 0.9) + (e.fearful * 0.8) + (e.disgusted * 0.5));
  ...
  init() {
    ...
    this.chart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: { x: { display: false }, y: { min: -1, max: 1, ticks: { color: '#8b92a8' } } },
        plugins: { legend: { display: false }, annotation: { annotations: { playhead: { type: 'line', xMin: 0, xMax: 0, borderColor: '#a855f7', borderWidth: 2 } } } }
      }
    });
  }
  update(metrics) {
...
class NeuroVisApp {
  ...
  _updateUI(avg) {
    const labels = { happy: '😊 Feliz', sad: '😢 Triste', angry: '😠 Enojo', surprised: '😲 Sorpresa', fearful: '😨 Miedo', disgusted: '🤢 Disgusto', neutral: '😐 Neutral' };
    ...
    document.getElementById('val-dominant').textContent = labels[avg.dominant] || 'Neutral';
  }
